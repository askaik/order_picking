frappe.pages['order_picking'].on_page_load = function (wrapper) {
	...
	const $submit_pick_btn = $('#submit_pick_btn');
	...
	function update_progress() {
		...
		if (percentage === 100) {
			...
			frappe.show_alert({ message: __('Invoice Fully Picked!'), indicator: 'green' });
			$item_input.prop('disabled', true);
			$ready_btn.show();
			$submit_pick_btn.show();
		} else {
	...
	$ready_btn.on('click', function () {
		if (current_invoice) {
			frappe.call({
				...
				callback: function (r) {
					if (!r.exc) {
						frappe.show_alert({ message: __('Invoice attached to Order Pick & Marked Ready!'), indicator: 'green' });
						// Reset UI for next invoice
	...
	// Request Dispatch / Submit Session 
	$submit_pick_btn.on('click', function () {
		if (order_pick_id) {
			frappe.confirm(__('Submit this Order Pick? This will create a Dispatch Order.'), () => {
				frappe.call({
					...
					callback: function (r) {
						if (!r.exc) {
							frappe.show_alert({ message: __('Order Pick Submitted & Dispatch Order created!'), indicator: 'green' });
							// Reload page or session
