...
class OrderPick(Document):
	def on_submit(self):
		...
		try:
			...
			frappe.log_error(frappe.get_traceback(), "Order Pick Submit: Dispatch Order Creation Failed")
			frappe.throw(frappe._("Failed to create Dispatch Order: {0}").format(str(e)))
