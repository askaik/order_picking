<template>
  <div class="picking-wrapper max-w-5xl mx-auto p-5">
    ...
    <div class="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 mb-6 relative overflow-hidden">
      ...
      <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center mt-4 gap-4">
        <div class="flex flex-wrap items-center gap-3 w-full xl:w-auto">
          <div class="text-sm font-medium text-gray-500 bg-gray-50 dark:bg-slate-700 dark:text-slate-200 px-3 py-1.5 rounded-md border border-gray-100 dark:border-slate-600 shadow-sm whitespace-nowrap" v-if="currentInvoice">
              Active: <span class="font-bold text-gray-800 dark:text-white">{{ currentInvoice }}</span>
          </div>
          <div v-if="currentInvoiceStatus" :class="statusColorClass" class="text-xs font-bold px-2.5 py-1.5 rounded-md uppercase tracking-wider shadow-sm border whitespace-nowrap">
              {{ currentInvoiceStatus }}
          </div>
          
          <div v-if="currentInvoiceStatus === 'Draft'" class="flex flex-wrap items-center gap-3">
			  <div class="text-xs font-semibold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 px-3 py-1.5 rounded-md border border-amber-200 dark:border-amber-800 flex items-center gap-1.5 animate-[pulse_2s_ease-in-out_infinite]">
				...
				<span>Please Consider Submitting The Invoice</span>
			  </div>
			  <button @click="submitInvoice" :disabled="isSubmittingInvoice" class="bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold py-1.5 px-3 rounded-md shadow-sm transition-colors flex items-center gap-1.5 disabled:opacity-50 whitespace-nowrap">
				...
				<svg v-else class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
        ...
        <div class="flex flex-wrap items-center gap-3 w-full xl:w-auto xl:justify-end">
            <button 
                ...
                @click="markReady" 
                class="bg-green-600 hover:bg-green-700 active:bg-green-800 text-white font-bold py-2.5 px-6 rounded-lg text-sm shadow-md transition-all hover:-translate-y-0.5 hover:shadow-lg flex items-center justify-center gap-2 whitespace-nowrap min-w-[170px]"
            >
            ...
            <button 
                ...
                class="bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white font-bold py-2.5 px-6 rounded-lg text-sm shadow-md transition-all hover:-translate-y-0.5 hover:shadow-lg flex items-center justify-center gap-2 whitespace-nowrap min-w-[170px]"
            >
